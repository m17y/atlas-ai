...
export default function TrendingPage() {
  ...
  const allCategories = ['全部', ...categories.map(c => (c as any).name || c.name || c)]

  const filteredTools = selectedCategory && selectedCategory !== '全部'
    ? tools.filter(tool => (tool.category as any)?.name === selectedCategory || tool.category === selectedCategory)
    : tools

  // Sort by hotness (reviewCount) for trending
